...
We use **KaTeX** for rendering. See MathematicalExpressions.md for the full syntax guide.

### 3.1. Syntax

*   **Inline:** Use single dollars: `$ E = mc^2 $`
*   **Block (Simple):** Use double dollars for unnumbered equations:
    ```md
    $$
    \int_a^b f(x) dx
    $$
...
Use `\tag{}` inside the LaTeX block for visual numbering.
...
**Step 2: Link to the Equation**
Use a standard Markdown link pointing to the `id`.

```md
Substituting into [Equation 1](#eq:pythagoras)...
```

**Note:** Always use a consistent prefix like `eq:` for equation IDs to avoid collisions with section headers.
...
1.  **Define the logic:** Create a function in the corresponding page script (e.g., `src/scripts/pages/geometryPage.js`) inside the exported dictionary.
2.  **Insert the graphic:** Use the `vector:` protocol followed by the ID defined in your page script.
...
### 7.2. Referencing Equations (Hybrid)

Standard Markdown does not support assigning `id`s to arbitrary blocks (like equations). Therefore, we use the `<Equation>` component to define the target.

**Step 1: Label the Equation**
Wrap the equation block in an `<Equation>` component with a unique `id`. Use `\tag{}` for visual numbering.

```jsx
<Equation id="eq:pythagoras">
$$
a^2 + b^2 = c^2 \tag{1}
$$
</Equation>
...
**Step 2: Link to the Equation (Markdown)**
Use a standard Markdown link pointing to the `id`.
